...
const translations = {
  en: {
    ...
    'hero.p':            'Ping and traceroute to hundreds of cloud endpoints — natively, with no external binaries. Filter by location, sort by distance, measure everything.',
    ...
    'f6.name': 'Data Visualization',
    'f6.desc': 'Measurements rendered in an optimized monospace table layout. Dense, readable, precise.',
  ...
  pt: {
    ...
    'hero.p':            'Ping e traceroute para centenas de endpoints na nuvem — nativamente, sem binários externos. Filtre por localização, ordene por distância, meça tudo.',
    ...
    'f6.name': 'Visualização de Dados',
    'f6.desc': 'Medições renderizadas em um layout de tabela monospace otimizado. Denso, legível, preciso.',
  ...
  es: {
    ...
    'hero.p':            'Ping y traceroute a cientos de endpoints en la nube — de forma nativa, sin binarios externos. Filtra por ubicación, ordena por distancia, mide todo.',
    ...
    'f6.name': 'Visualización de Datos',
    'f6.desc': 'Mediciones renderizadas en un diseño de tabla monospace optimizado. Denso, legible, preciso.',
  ...
  de: {
    ...
    'hero.p':            'Ping und Traceroute zu Hunderten von Cloud-Endpunkten — nativ, ohne externe Binärdateien. Nach Standort filtern, nach Entfernung sortieren, alles messen.',
    ...
    'f6.name': 'Datenvisualisierung',
    'f6.desc': 'Messungen in einem optimierten Monospace-Tabellenlayout. Dicht, lesbar, präzise.',
  ...
  fr: {
    ...
    'hero.p':            'Ping et traceroute vers des centaines d\'endpoints cloud — nativement, sans binaires externes. Filtrez par emplacement, triez par distance, mesurez tout.',
    ...
    'f6.name': 'Visualisation des données',
    'f6.desc': 'Mesures affichées dans une mise en page de tableau monospace optimisée. Dense, lisible, précis.',
  ...
  nl: {
    ...
    'hero.p':            'Ping en traceroute naar honderden cloud-endpoints — native, zonder externe binaire bestanden. Filter op locatie, sorteer op afstand, meet alles.',
    ...
    'f6.name': 'Gegevensvisualisatie',
    'f6.desc': 'Metingen weergegeven in een geoptimeerde monospace-tabelindeling. Dicht, leesbaar, nauwkeurig.',
  ...
  ru: {
    ...
    'hero.p':            'Ping и traceroute к сотням облачных эндпоинтов — нативно, без внешних бинарных файлов. Фильтруйте по местоположению, сортируйте по расстоянию, измеряйте всё.',
    ...
    'f6.name': 'Визуализация данных',
    'f6.desc': 'Измерения отображаются в оптимизированной монопространственной таблице. Насыщенно, читаемо, точно.',
  ...
  sr: {
    ...
    'hero.p':            'Ping i traceroute do stotina cloud endpoint-a — nativno, bez eksternih fajlova. Filtrirajte po lokaciji, sortirajte po udaljenosti, merite sve.',
    ...
    'f6.name': 'Vizualizacija podataka',
    'f6.desc': 'Merenja prikazana u optimizovanom monospace rasporedu tabele. Gusto, čitljivo, precizno.',
  ...
  ar: {
    ...
    'hero.p':            'أوامر Ping وTraceroute لمئات من نقاط النهاية السحابية — بشكل أصلي، دون ثنائيات خارجية. فلتر حسب الموقع، رتب حسب المسافة، قس كل شيء.',
    ...
    'f6.name': 'تصور البيانات',
    'f6.desc': 'قياسات معروضة في تخطيط جدول monospace محسّن. كثيف، مقروء، دقيق.',
  ...
  zh: {
    ...
    'hero.p':            '对数百个云端点执行 Ping 和路由追踪 — 原生执行，无需外部二进制文件。按位置过滤，按距离排序，测量一切。',
    ...
    'f6.name': '数据可视化',
    'f6.desc': '测量结果以优化的等宽字体表格布局渲染。密集、可读、精确。',
  ...
  ja: {
    ...
    'hero.p':            '数百のクラウドエンドポイントへの Ping と Traceroute — ネイティブで実行、外部ファイル不要。場所でフィルタリング、距離でソート、すべてを測定。',
    ...
    'f6.name': 'データ可視化',
    'f6.desc': '最適化されたモノスペースのテーブルレイアウトで測定結果を表示。密度が高く、読みやすく、正確。',
  ...
  ko: {
    ...
    'hero.p':            '수백 개의 클라우드 엔드포인트에 대한 Ping과 Traceroute — 네이티브로 실행, 외부 파일 없음. 위치로 필터링하고, 거리로 정렬하고, 모든 것을 측정하세요.',
    ...
    'f6.name': '데이터 시각화',
    'f6.desc': '최적화된 고정폭 테이블 레이아웃으로 측정값 렌더링. 밀도 높고, 읽기 쉽고, 정확.',
